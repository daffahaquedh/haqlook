...
function ListingsPage() {
  ...
  return <div><SellerHeader eyebrow="DISTRIBUTION STATUS" title="Listings" copy="Review marketplace copy and track where each master inventory item is listed. Posting and removal remain manual." />{message && <Notice tone="error">{message}</Notice>}<section className="seller-panel"><PanelTitle eyebrow="ALL CHANNELS" title="Marketplace listings" />{items.length ? <div className="listing-table">{items.map((listing) => { const safeUrl = safeHttpUrl(listing.listing_url); return <div className="listing-row wide-row" key={listing.id}><img src={imageFor(listing.products)} alt="" loading="lazy" /><div><strong>{listing.products?.sku || 'SKU pending'} · {listing.listing_title || listing.products?.name || 'Unknown item'}</strong><span>{titleCaseStatus(listing.marketplace)} · {listing.listing_description || (safeUrl ? 'Listing URL saved' : 'No URL saved')}</span>{safeUrl && <a className="seller-text-link" href={safeUrl} target="_blank" rel="noreferrer">Open marketplace ↗</a>}{listing.products?.status === 'sold' && listing.listing_status === 'LISTED' && <em className="listing-active-warning">Item sold — check this external listing manually.</em>}</div><b className={`listing-status ${listing.listing_status.toLowerCase()}`}>{titleCaseStatus(listing.listing_status)}</b><small>{listing.listed_price ? moneyIdr(listing.listed_price) : '—'}</small></div> })}</div> : <div className="seller-empty"><strong>NO LISTINGS RECORDED</strong><p>Open an inventory item to update its marketplace status.</p></div>}</section></div>
}
