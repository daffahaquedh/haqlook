...
export function HunterChatPage() {
  ...
  return <div className="hunter-page">
    <header className="hunter-page-head"><div><span className="hunter-eyebrow">HAQLOOKS SOURCING COPILOT</span><h1>Riset</h1><p>Asisten sourcing Haqlooks · Haqlooks sourcing assistant</p></div><div className="hunter-session-actions"><label className="sr-only" htmlFor="hunter-session">Pilih sesi Hunter</label><select id="hunter-session" value={sessionId} onChange={(event) => { const selected = sessions.find((item) => item.id === event.target.value); if (selected) void selectSession(selected) }}>{sessions.map((session) => <option key={session.id} value={session.id}>{session.destination || 'Percakapan baru'}</option>)}</select><button className="hunter-icon-button" type="button" onClick={() => void newSession()} aria-label="Mulai percakapan baru">＋</button></div></header>
