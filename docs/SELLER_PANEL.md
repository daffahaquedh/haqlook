...
`/seller/ai-hunter` starts with “Hari ini mau hunting ke mana?” and quick destination choices. A deterministic preflight collects destination, budget, category, and market goal locally; destination research is not requested until the seller reviews the plan and presses `Mulai Research`. The authenticated `seller-ai` function also rejects unconfirmed research and non-reasoning chat requests. Destination research uses the Responses API with `gpt-6-luna`, reasoning `medium`, built-in `web_search`, and a ten-tool-call cap. Complex chat and photo checks use reasoning `low` and never receive a web-search tool.

Destination briefs are shared by destination for 12 hours. A confirmed repeat request reads the unexpired database cache before checking for an OpenAI key or reserving usage; a cache hit creates no AI usage row. Only an explicitly confirmed refresh bypasses that cache. Preflight, greetings, short answers, category/budget/market/section filters, and sorting use deterministic local code. Complex chat is limited to low-reasoning, no-search questions and receives at most three relevant targets with their matching evidence, not the full conversation or brief. Public URLs are retained only when returned in actual Responses citation annotations or `web_search_call.action.sources`. Prices and max-buy values are withheld if their source/rationale checks fail. Every item target is labeled a `SOURCING_HYPOTHESIS`; the feature never claims current physical stock or photo-based authenticity.
