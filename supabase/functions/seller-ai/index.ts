...
Deno.serve(async (request) => {
  ...
  const input = [{
    ...
    content: [
      {
        type: 'input_text',
        text: `Analyze this pre-owned fashion item for a human seller. Return only the required JSON schema. Write every primary text field without the _en suffix in Bahasa Indonesia, then provide its concise English equivalent in the matching _en field. Keep brand and marketplace names unchanged. For paired arrays, keep the same item order. Unknown values must be empty strings or empty arrays. Use authenticity_note exactly as a caution that authenticity has not been verified and manual review is required; never determine or imply authenticity from photos. Do not invent market facts, prices, or provenance. Give a brief suitability recommendation (for example sangat cocok, cocok, kurang cocok) in Indonesian and an equivalent in English. Marketplace recommendations must cover Haqlooks, Preloved, Grailed, Vestiaire, Carousell, and Instagram. Product data only; do not request or infer buyer names, emails, phone numbers, addresses, passwords, payment data, or marketplace credentials.\n\nPRODUCT DATA: ${JSON.stringify(safeItem)}`,
      },
